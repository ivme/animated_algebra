...
void stack_action::create_children() {
	...
	auto fcm = this->frame_count_method;
	action::callback_func add_settle_down = [fcm,settle_dimension_](std::shared_ptr<action> parent, std::shared_ptr<action> finished_action) {
		auto settle_down = std::make_shared<settle>(finished_action->get_node(),parent->get_current_frame() + 1,settle_dimension_);
		//settle_down->use_frame_count_method(fcm);
		parent->add_child(settle_down);
		std::dynamic_pointer_cast<move>(parent)->global_update();
	};

	action::callback_func add_space_children2 = [fcm,add_settle_down,dx2_,dy2_](std::shared_ptr<action> parent, std::shared_ptr<action> finished_action) {
	...
	if (initial_spacing != 0) {
		...
		space_children1->use_frame_count_method(fcm);
		global_update();
	} else {
