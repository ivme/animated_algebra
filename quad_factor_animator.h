...
public:
	...
	virtual std::shared_ptr<liven::animation> animate() override;
...
private:
	...
	std::vector<std::shared_ptr<p_rect>> cols;
	std::shared_ptr<liven::node> col1_node, col2_node, col2a_node, col2b_node, col3_node;
	std::shared_ptr<liven::node> p_rect_nodes, group1, group2, group2a, group2b, group3;
	std::string var_name;
	int var_val;
	std::shared_ptr<liven::text_node> quad_expr;
	std::shared_ptr<liven::node> quad_expr_node;
